...
void BlueLightFilter(void * const topaddr, const int bufferwidth, const int pixelformat) {
	...
	// It was easier to do the maths in terms of bytes so using void pointers
	for (void* current_word_row = topaddr; current_word_row < topaddr + buffer_size_bytes; current_word_row += bytes_per_buffer_row) {
		for (void* current_word = current_word_row; current_word < current_word_row + screen_width_bytes; current_word+=sizeof(uint32_t)) {
